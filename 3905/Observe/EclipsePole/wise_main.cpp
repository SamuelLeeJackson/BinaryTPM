...
double EPS = 0.9;													// Asteroid emissivity
...
double hel_polar0 = 1.977789;											// Heliocentric distance in AU of asteroid position
double hel_polar1 = ((11.3684/180.0)*PI);							// Heliocentric ecliptic latitude in radians of asteroid position
double hel_polar2 = ((110.6920/180.0)*PI);							// Heliocentric ecliptic longitude in radians of asteroid position 
double obs_polar0 = 1.631629;											// Geocentric distance in AU of asteroid position
double obs_polar1 = ((13.826432/180.0)*PI);							// Geocentric ecliptic latitude in radians of asteroid position
double obs_polar2 = ((80.190885/180.0)*PI);							// Geocentric ecliptic longitude in radians of asteroid position
double rot0 = ((60.0/180.0)*PI);									// Helicentric ecliptic latitude in radians of asteroid rotation pole
double rot1 = ((221.0/180.0)*PI);									// Helicentric ecliptic longitude in radians of asteroid rotation pole

int observation_set_number = 0;										// Data set number (start at 0 and increment for each unique observation geometry/set)
int minimum_thermal_inertia = 0;									// Minimum thermal inertia value to run model for
int maximum_thermal_inertia = 500;									// Maximum thermal inertia value to run model for (set to minimum thermal inertia value if you want to run for a single and fixed thermal inertia value)
...
void binary_loadsurfacevisibility(Surface &S, Asteroid &A, string rough_visibility_filename) {
	...
	string test_str = "test.txt";
	ofstream test(test_str.c_str());
	float pos[3];
	int vert[3];
	float val[NTAU];
...
int main() {
	...
	int num_TI_steps, t_inertia;
	num_TI_steps=(maximum_thermal_inertia-minimum_thermal_inertia)/thermal_inertia_step;

	for (int TI_step=0;TI_step!=(num_TI_steps+1);++TI_step) {
...
//	ofstream write("2002CE26_flux_map(170).txt");
//	for (AstFacet *C=A.beginf();C!=A.endf();++C) {
//		write << C->rtnsmooth_flux(0) << "\t" << C->rtnrough_flux(0) << endl;
//	}
